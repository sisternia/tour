  ...
  if (form) {
    if (guideId) {
      ...
      if (isEditMode) {
        // --- Edit Mode ---
        if (pageTitle)
          pageTitle.textContent = "Chỉnh sửa thông tin Hướng Dẫn Viên";
        ...
        // --- View Mode ---
        if (pageTitle)
          pageTitle.textContent = "Thông tin chi tiết Hướng Dẫn Viên";
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (verifiesInput)
        formData.append("verifies_status", verifiesInput.value);

      // Thu thập Kỹ năng
      const selectedLanguages = Array.from(
        document.querySelectorAll('input[name="guide_languages"]:checked'),
      ).map((cb) => cb.value);
      const selectedFields = Array.from(
        document.querySelectorAll('input[name="guide_fields"]:checked'),
      ).map((cb) => cb.value);
      ...
      try {
        ...
        const url = isUpdating
          ? `/api/guides/update-guide/${guideId}`
          : "/api/guides/create-guide";
        ...
        if (result.success) {
          showNotification(
            isUpdating ? "Cập nhật thành công!" : "Thêm thành công!",
            "success",
          );
          setTimeout(() => {
...
async function fetchAddGuideSkills(selectedLangs = [], selectedFields = []) {
  try {
    ...
    if (langContainer && langData.success) {
      ...
      if (isViewMode) {
        languagesToRender = languagesToRender.filter((lang) =>
          selectedLangs.includes(lang.guide_lan_id),
        );
      }
      ...
      langContainer.innerHTML =
        languagesToRender
          .map((lang) => {
            const isChecked = selectedLangs.includes(lang.guide_lan_id)
              ? "checked"
              : "";
          ...
          .join("") ||
        `<p class="text-xs text-on-surface-variant italic">${isViewMode ? "Chưa chọn ngôn ngữ nào." : "Chưa có ngôn ngữ nào trong hệ thống."}</p>`;
    }
    ...
    if (fieldContainer && fieldData.success) {
      ...
      if (isViewMode) {
        fieldsToRender = fieldsToRender.filter((field) =>
          selectedFields.includes(field.guide_fie_id),
        );
      }
      ...
      fieldContainer.innerHTML =
        fieldsToRender
          .map((field) => {
            const isChecked = selectedFields.includes(field.guide_fie_id)
              ? "checked"
              : "";
            const isDisabled = isViewMode ? "disabled" : "";
            return `
          ...
          .join("") ||
        `<p class="text-xs text-on-surface-variant italic">${isViewMode ? "Chưa chọn lĩnh vực nào." : "Chưa có lĩnh vực nào trong hệ thống."}</p>`;
    }
  } catch (error) {
...
async function fetchGuideDetails(id) {
  try {
    ...
    if (result.success) {
      ...
      // Trạng thái xác nhận
      const verifiesStatus =
        guide.verifies_status !== undefined ? guide.verifies_status : 0;
      if (typeof updateVerifiesStatus === "function") {
            ...
            <tr class="group hover:bg-surface-container-high transition-all duration-200">
                ...
                <td class="px-8 py-6">
                    <div class="flex items-center justify-center gap-2">
                        <button onclick="window.location.href='guide_detail.html?id=${guide.user_id}'" class="w-9 h-9 flex items-center justify-center bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all shadow-sm" title="Xem">
                            <span class="material-symbols-outlined text-[20px]">visibility</span>
                        </button>
                        <button onclick="window.location.href='guide_detail.html?id=${guide.user_id}&edit=true'" class="w-9 h-9 flex items-center justify-center bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all shadow-sm" title="Sửa">
...
window.updateVerifiesStatus = function (status) {
  ...
  if (status == 1) {
    btn1.className =
      baseClass + " bg-green-500 text-white shadow-lg shadow-green-200";
  } else {
    btn0.className =
      baseClass + " bg-red-500 text-white shadow-lg shadow-red-200";
  }
};
