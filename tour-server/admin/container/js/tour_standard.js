document.addEventListener("DOMContentLoaded", function () {
  ...
  window.addEventListener("paste", function (e) {
    ...
    if (pastedFiles.length > 0) {
      ...
      if (isModalOpen) {
        scheSelectedFiles = [...scheSelectedFiles, ...pastedFiles];
        renderScheImagePreviews();
        if (typeof showNotification === "function")
          showNotification(`Đã dán ${pastedFiles.length} ảnh vào hoạt động`);
      } else {
        const step1 = document.getElementById("step-1-content");
        if (step1 && !step1.classList.contains("hidden")) {
          window.selectedFiles = [...window.selectedFiles, ...pastedFiles];
          window.renderImagePreviews();
          if (typeof showNotification === "function")
            showNotification(`Đã dán ${pastedFiles.length} ảnh vào tour`);
        }
      }
    }
  });
  ...
  setupDragAndDrop("upload-zone", (files) => {
    window.selectedFiles = [...window.selectedFiles, ...files];
    window.renderImagePreviews();
    if (typeof showNotification === "function")
      showNotification(`Đã thêm ${files.length} ảnh vào tour`);
  });

  setupDragAndDrop("sche-upload-zone", (files) => {
    scheSelectedFiles = [...scheSelectedFiles, ...files];
    renderScheImagePreviews();
    if (typeof showNotification === "function")
      showNotification(`Đã thêm ${files.length} ảnh vào hoạt động`);
  });
  ...
  window.submitTour = async function () {
    ...
    const guides = Array.from(
      document.querySelectorAll('input[name="guide_select"]:checked'),
    ).map((cb) => cb.value);
    ...
    const tourStatus = document.getElementById("tour-status-select")
      ? document.getElementById("tour-status-select").value
      : "Bản nháp";
    ...
    const url = editingTourId
      ? `/api/tours/update-tour/${editingTourId}`
      : "/api/tours/create-tour";
    ...
    try {
      ...
      if (result.success) {
        const successMsg = editingTourId
          ? "Sửa thành công!"
          : "Thêm thành công!";
  ...
  window.calculateEndDate = function () {
    ...
    durationInfo.textContent =
      days === 1 ? "1 ngày (Trong ngày)" : `${days} ngày ${nights} đêm`;
  };
  ...
  window.generateTimeline = function () {
    ...
    allSchedules.forEach((sche) => {
      const container = document.getElementById(
        `day-${sche.day_number}-activities`,
      );
      if (container) {
        ...
        if (sche.images && sche.images.length > 0) {
          ...
          Promise.all(promises).then((urls) => {
            container.insertAdjacentHTML(
              "beforeend",
              renderActivityItemHTML(sche, urls),
            );
          });
        } else {
          container.insertAdjacentHTML(
            "beforeend",
            renderActivityItemHTML(sche),
          );
        }
      }
    });
  ...
  window.renderActivityItemHTML = function (
    sche,
    imgUrls = [],
    readOnly = false,
  ) {
  ...
  window.updateTourStatusBadge = function () {
    ...
    if (val === "Bản nháp") {
      ...
      dotElem.className =
        "absolute left-3 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-rose-500 animate-pulse pointer-events-none z-20";
    }
  };
  ...
  window.goToStep = function (step) {
    ...
    if (step === 4) {
      const info = getBasicInfo();
      document.getElementById("review-tour-name").textContent =
        info.tour_name || "Tên Tour chưa đặt";
      document.getElementById("review-tour-desc").textContent =
        info.tour_desc || "Chưa có mô tả chi tiết.";
      document.getElementById("review-tour-type").textContent =
        `Tour Cao cấp ${info.tour_type}`;
      document.getElementById("review-adult-price").textContent =
        info.price_adult || "0";
      document.getElementById("review-child-price").textContent =
        info.price_child || "0";
      document.getElementById("review-capacity").textContent =
        info.tour_capacity || "0";
      document.getElementById("review-duration-text").textContent =
        durationDisplay.textContent;
      document.getElementById("review-start-date").textContent =
        startDateInput.value
          ? new Date(startDateInput.value).toLocaleDateString("vi-VN")
          : "--/--/----";
      document.getElementById("review-end-date").textContent =
        endDateInput.value
          ? new Date(endDateInput.value).toLocaleDateString("vi-VN")
          : "--/--/----";
      document.getElementById("review-location").textContent =
        info.tour_add || "Chưa xác định";
      ...
      if (info.images.length > 0) {
        ...
        if (subPreview) {
          ...
          info.images.slice(0, 3).forEach((img, idx) => {
            ...
            d.className =
              "aspect-square rounded-lg overflow-hidden bg-surface-container-low border border-outline-variant/5";
      ...
      if (itineraryFlow) {
        ...
        const renderAllReview = async () => {
          ...
          const tourStartDate = startDateInput.value
        ...
        window.toggleDayActivities = function (day) {
          ...
          const shouldCollapse =
            firstContent && !firstContent.classList.contains("hidden");
      ...
      if (guideContainer) {
        if (selectedGuideIds.length > 0) {
          guideContainer.innerHTML = selectedGuideIds
            .map((guideId) => {
              const guide = allGuides.find(
                (g) => (g.user_id?.toString() || g.user_id) == guideId,
              );
              if (!guide) return "";
    ...
    document.querySelectorAll(".step-indicator").forEach((indicator) => {
      ...
      if (s < step) {
        ...
        icon.className =
          "step-icon w-10 h-10 rounded-full bg-primary text-on-primary flex items-center justify-center font-bold";
        label.className = "step-label text-xs font-bold text-primary";
      } else if (s === step) {
        icon.innerHTML = s;
        icon.className =
          "step-icon w-10 h-10 rounded-full bg-primary text-on-primary flex items-center justify-center font-bold shadow-lg shadow-primary/20";
        label.className = "step-label text-xs font-bold text-primary";
      } else {
        icon.innerHTML = s;
        icon.className =
          "step-icon w-10 h-10 rounded-full bg-surface-container-high text-on-surface-variant flex items-center justify-center font-bold";
        label.className =
          "step-label text-xs font-medium text-on-surface-variant";
      }
    });
  ...
  function renderGuides() {
    ...
    allGuides.forEach((guide) => {
      ...
      const dobStr = guide.dob
        ...
        : "--/--/----";
      const langs =
        guide.languages && guide.languages.length > 0 ? guide.languages : [];
      const fields =
        guide.fields && guide.fields.length > 0 ? guide.fields : [];
  ...
  function initReviewMap(lat, lng) {
    ...
    if (!reviewMap) {
      ...
      reviewMap.eachLayer((layer) => {
        if (layer instanceof L.Marker) reviewMap.removeLayer(layer);
      });
    }
  ...
  function ensureActivityMapInitialized() {
    if (!activityMap && typeof L !== "undefined") {
      ...
      if (actMapElem) {
        ...
        activityMap.on("click", (e) =>
          setActivityLocation(e.latlng.lat, e.latlng.lng),
        );
      }
    }
  }
  ...
  window.openActivityModal = function (dayIndex) {
    ...
    document.getElementById("modal-day-label").textContent =
      `Ngày ${dayIndex.toString().padStart(2, "0")} - Lịch trình`;
    document
      .getElementById("activity-continue-btn")
      .querySelector("span").textContent = "Thêm & Tiếp tục";
  ...
  window.openEditActivityModal = function (tempId) {
    ...
    document
      .getElementById("activity-continue-btn")
      .querySelector("span").textContent = "Lưu & Tiếp tới";
  ...
  window.searchActivityLocation = async function () {
    ...
    if (data.length > 0)
      setActivityLocation(parseFloat(data[0].lat), parseFloat(data[0].lon));
  };
  ...
  window.saveActivity = async function (isContinue = false) {
    ...
    const scheObj = {
      ...
      tour_sche_longit: !isNaN(
        parseFloat(document.getElementById("act-lng").textContent),
      )
        ? parseFloat(document.getElementById("act-lng").textContent)
        : null,
      tour_sche_latit: !isNaN(
        parseFloat(document.getElementById("act-lat").textContent),
      )
        ? parseFloat(document.getElementById("act-lat").textContent)
        : null,
    ...
    try {
      ...
      const container = document.getElementById(
        `day-${currentDayForActivity}-activities`,
      );
      if (container) {
        container.innerHTML = "";
        const dayActivities = allSchedules.filter(
          (s) => s.day_number == currentDayForActivity,
        );
  ...
  window.setTourType = function (type) {
    document.getElementById("tour-type").value =
      type === "intl" ? "Quốc tế" : "Nội địa";
    document.getElementById("btn-tour-intl").className =
      type === "intl"
        ? "flex-1 flex items-center justify-center gap-2 rounded-md bg-surface-container-lowest text-primary font-bold shadow-sm transition-all"
        : "flex-1 flex items-center justify-center gap-2 rounded-md text-on-surface-variant font-medium hover:text-on-surface transition-all";
    document.getElementById("btn-tour-domestic").className =
      type === "domestic"
        ? "flex-1 flex items-center justify-center gap-2 rounded-md bg-surface-container-lowest text-primary font-bold shadow-sm transition-all"
        : "flex-1 flex items-center justify-center gap-2 rounded-md text-on-surface-variant font-medium hover:text-on-surface transition-all";
  };
  ...
  function renderTours(tours) {
    ...
    if (tours.length === 0) {
      tourTableBody.innerHTML =
        '<tr><td colspan="8" class="px-6 py-10 text-center text-on-surface-variant italic">Chưa có tour nào được tạo.</td></tr>';
      return;
    }
    ...
    tours.forEach((tour) => {
      ...
      row.className =
        "hover:bg-surface-container-high transition-colors group border-b border-surface-container last:border-none";

      const startDate = tour.time.date_start
        ? new Date(tour.time.date_start).toLocaleDateString("vi-VN", {
            day: "2-digit",
            month: "2-digit",
          })
        : "--/--";
      const endDate = tour.time.date_end
      ...
      const priceAdult = tour.price.price_adult
        ? tour.price.price_adult.toLocaleString("vi-VN")
        : "0";
      const priceChild = tour.price.price_child
        ? tour.price.price_child.toLocaleString("vi-VN")
        : "0";
      ...
      const guidesHtml =
        tour.guides.length > 0
          ? tour.guides
              .map(
                (g) => `
                    <div class="flex items-center justify-center mb-0.5 last:mb-0 w-full">
                        <span class="text-[11px] font-semibold text-on-surface/80 truncate px-1" title="${g.full_name}">${g.full_name}</span>
                    </div>
                `,
              )
              .join("")
      ...
      row.innerHTML = `
                <td class="px-6 py-4">
                    <div class="flex items-center gap-4 whitespace-nowrap max-w-[300px]">
                        ...
                        <div class="flex flex-col min-w-0">
                            <p class="text-[13px] font-extrabold text-on-surface font-headline leading-tight group-hover:text-primary transition-colors truncate" title="${tour.tour_name}">${tour.tour_name}</p>
                            <div class="flex items-center gap-2 mt-1 whitespace-nowrap">
                                <span class="text-[9px] font-black bg-primary/10 text-primary px-1.5 py-0.5 rounded uppercase tracking-tighter shrink-0">${tour.tour_type}</span>
                                <span class="text-[9px] font-bold text-on-surface-variant/40 uppercase tracking-widest truncate">ID: ${tour.tour_id.slice(-6)}</span>
                            </div>
                        </div>
                    </div>
                </td>
                    ...
                    <div class="flex flex-col items-center justify-center gap-1 whitespace-nowrap">
                        ...
                        ${
                          tour.available_slots <= 0
                            ? '<span class="text-[8px] font-black uppercase text-red-600 bg-red-50 px-1 rounded whitespace-nowrap">Hết chỗ</span>'
                            : `<span class="text-[8px] font-bold text-emerald-600 bg-emerald-50 px-1 rounded whitespace-nowrap">Còn ${tour.available_slots} chỗ</span>`
                        }
                    </div>
                </td>
                <td class="px-6 py-4 text-center">
                    <div class="flex flex-col gap-1.5 min-w-[100px] items-center whitespace-nowrap">
                        <div class="flex items-center justify-between w-full bg-primary/5 px-2 py-1 rounded-md border border-primary/10 whitespace-nowrap">
                        ...
                        <div class="flex items-center justify-between w-full bg-on-surface-variant/5 px-2 py-1 rounded-md border border-on-surface-variant/10 whitespace-nowrap">
                ...
                <td class="px-6 py-4 text-center">
                    <div class="flex flex-col items-center gap-0.5 whitespace-nowrap max-w-[150px] mx-auto">
                ...
                <td class="px-6 py-4 text-center">
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm ring-1 ring-inset whitespace-nowrap ${
  ...
  window.deleteTour = function (id) {
    showConfirmModal(
      "Bạn có chắc chắn muốn xóa tour này và toàn bộ dữ liệu liên quan không?",
      async () => {
        try {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (tour.tour_latit && tour.tour_longit) {
          setLocation(
            parseFloat(tour.tour_latit),
            parseFloat(tour.tour_longit),
          );
        }

        // Duration & Prices
        document.getElementById("duration-days").value =
          tour.time.tour_duration;
        document.getElementById("price-adult").value =
          tour.price.price_adult.toLocaleString("vi-VN");
        document.getElementById("price-child").value =
          tour.price.price_child.toLocaleString("vi-VN");
        document.getElementById("tour-capacity").value =
          tour.price.tour_capacity;
        document.getElementById("start-date").value =
          tour.time.date_start.split("T")[0];
        updateDuration();
        ...
        if (statusSelect && tour.tour_status) {
          ...
          if (typeof window.updateTourStatusBadge === "function")
            window.updateTourStatusBadge();
        }
        ...
        tour.guides.forEach((g) => {
          if (
            !allGuides.find(
              (ag) => (ag.user_id?.toString() || ag.user_id) == g.user_id,
            )
          ) {
            allGuides.push(g);
          }
